...
class FruitQuality():
    ...
    dd_cum = xs.foreign(branching.Identity, 'dd_cum')
    ...
    def initialize(self):
        ...
        self.FM_fruit = np.zeros(self.branch.shape)
        self.W_fleshpeel = np.zeros(self.branch.shape)
        self.DM_fleshpeel = np.zeros(self.branch.shape)
        self.W_flesh = np.zeros(self.branch.shape)
        self.DM_flesh = np.zeros(self.branch.shape)
        self.water_potential_fruit = np.zeros(self.branch.shape)
        self.turgor_pressure_fruit = np.zeros(self.branch.shape)
        self.osmotic_pressure_fruit = np.zeros(self.branch.shape)
        self.flux_xylem_phloem = np.zeros(self.branch.shape)
        self.transpiration_fruit = np.zeros(self.branch.shape)
        self.sucrose = np.zeros(self.branch.shape)
        self.glucose = np.zeros(self.branch.shape)
        self.fructose = np.zeros(self.branch.shape)
        self.soluble_sugars = np.zeros(self.branch.shape)
        self.starch = np.zeros(self.branch.shape)
        self.organic_acids = np.zeros(self.branch.shape)
        self.ripe = np.zeros(self.branch.shape)
    ...
    def run_step(self, step):
        ...
        sucrose_ripe_thresh = params.sucrose_ripe_thresh
        ...
        self.DM_flesh = np.ones(self.branch.shape) * e_fleshpeel2fleshDM * self.DM_fleshpeel
        ...
        DM_flesh_x_dd_cum = DM_flesh_previous * self.dd_cum
        prop_mal = np.maximum(0, delta_mal[0] + delta_mal[1] * self.dd_cum + delta_mal[2] * DM_flesh_previous + delta_mal[3] * DM_flesh_x_dd_cum)
        prop_cit = np.maximum(0, delta_cit[0] + delta_cit[1] * self.dd_cum + delta_cit[2] * DM_flesh_previous + delta_cit[3] * DM_flesh_x_dd_cum)
        prop_pyr = np.maximum(0, delta_pyr[0] + delta_pyr[1] * self.dd_cum + delta_pyr[2] * DM_flesh_previous + delta_pyr[3] * DM_flesh_x_dd_cum)
        prop_oxa = np.maximum(0, delta_oxa[0] + delta_oxa[1] * self.dd_cum + delta_oxa[2] * DM_flesh_previous + delta_oxa[3] * DM_flesh_x_dd_cum)
        prop_K = np.maximum(0, delta_K[0] + delta_K[1] * self.dd_cum + delta_K[2] * DM_flesh_previous + delta_K[3] * DM_flesh_x_dd_cum)
        prop_Mg = np.maximum(0, delta_Mg[0] + delta_Mg[1] * self.dd_cum + delta_Mg[2] * DM_flesh_previous + delta_Mg[3] * DM_flesh_x_dd_cum)
        prop_Ca = np.maximum(0, delta_Ca[0] + delta_Ca[1] * self.dd_cum + delta_Ca[2] * DM_flesh_previous + delta_Ca[3] * DM_flesh_x_dd_cum)
        prop_NH4 = np.maximum(0, delta_NH4[0] + delta_NH4[1] * self.dd_cum + delta_NH4[2] * DM_flesh_previous + delta_NH4[3] * DM_flesh_x_dd_cum)
        prop_Na = np.maximum(0, delta_Na[0] + delta_Na[1] * self.dd_cum + delta_Na[2] * DM_flesh_previous + delta_Na[3] * DM_flesh_x_dd_cum)
        prop_glc = np.maximum(0, delta_glc[0] + delta_glc[1] * self.dd_cum + delta_glc[2] * DM_flesh_previous + delta_glc[3] * DM_flesh_x_dd_cum)
        prop_frc = np.maximum(0, delta_frc[0] + delta_frc[1] * self.dd_cum + delta_frc[2] * DM_flesh_previous + delta_frc[3] * DM_flesh_x_dd_cum)
        prop_suc = np.maximum(0, delta_suc[0] + delta_suc[1] * self.dd_cum + delta_suc[2] * DM_flesh_previous + delta_suc[3] * DM_flesh_x_dd_cum)
        prop_sta = np.maximum(0, delta_sta[0] + delta_sta[1] * self.dd_cum + delta_sta[2] * DM_flesh_previous + delta_sta[3] * DM_flesh_x_dd_cum)
        ...
        Phi = phi_max * tau ** np.maximum(0., self.dd_cum - dd_thresh)

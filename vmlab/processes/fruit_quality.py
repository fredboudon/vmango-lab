...
class FruitQuality(ParameterizedProcess):
    """
    Process computing fruit composition in osmotically active solutes (sugars, acids and minerals)
    """
    ...
    def run_step(self):
        ...
        if np.any(self.nb_fruit > 0.):
            ...
            self.mass_mal[fruiting] = prop_mal * DM_flesh_previous
            nmol_mal = self.mass_mal[fruiting] / MM_mal
            self.mass_cit[fruiting] = prop_cit * DM_flesh_previous
            ...
            self.mass_glc[fruiting] = prop_glc * DM_flesh_previous
            nmol_glc = self.mass_glc[fruiting] / MM_glc
            self.mass_frc[fruiting] = prop_frc * DM_flesh_previous
            nmol_frc = self.mass_frc[fruiting] / MM_frc
            self.mass_suc[fruiting] = prop_suc * DM_flesh_previous
            nmol_suc = self.mass_suc[fruiting] / MM_suc
            self.mass_sta[fruiting] = prop_sta * DM_flesh_previous
